...
class OpAdmissionRegister(models.Model):
    ...
    x_admission_date = fields.Date('Fecha de admisión',readonly=True,
                    default=fields.Date.today(), states={'draft': [('readonly', False)]},
                    track_visibility='onchange')
    x_batch = fields.Many2one(
            'op.batch', 'Periodo', required=True, readonly=True,
            states={'draft':[('readonly', False)]}, track_visibility='onchange')
#new code
#    available_vacancies = fields.Integer(
#       'Available vacancies', readonly=True,
#        states={'draft': [('readonly', True)]}, default=30)
