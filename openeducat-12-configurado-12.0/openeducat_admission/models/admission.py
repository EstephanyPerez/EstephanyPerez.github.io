...
class OpAdmission(models.Model):
    ...
    x_age = fields.Integer('Edad', states={'done': [('readonly', True)]})
    x_grade = fields.Float('Nota',default=0.0)
    x_district = fields.Char('Distrito', size=128)
    x_student_identification_document = fields.Char('Documento de Identidad',size=15, required=True, 
            states={'done': [('readonly', True)], 'submit' : [('readonly', True)]})
    x_guardian_name = fields.Char('Nombre del apoderado',size=256,required=True)
    x_guardian_identification_document = fields.Char('Documento de Identidad',size=15,required=True)
    x_guardian_email = fields.Char('Correo del apoderado',size=128)
    x_guardian_mobile = fields.Char('Celular del apoderado',size=16,required=True)
    _sql_constraints = [
    ...
    def onchange_birth_date(self):
        if self.birth_date: 
            self.x_age = datetime.now().year - self.birth_date.year
    ...
    def onchange_student(self):
        if self.is_student and self.student_id:
            ...
            self.x_student_identification_document = sd.x_student_identification_document or False
            self.x_guardian_name = sd.x_guardian_name or False
            self.x_guardian_identification_document = sd.x_guardian_identification_document or False
            self.x_guardian_email = sd.x_guardian_email or False
            self.x_guardian_mobile = sd.x_guardian_mobile or False
            self.x_district = sd.x_district or False
        else:
            self.birth_date = ''
            self.gender = ''
            self.image = False
    ...
    def onchange_register(self):
        ...
        self.admission_date = self.register_id.x_admission_date

    @api.onchange('course_id')
    def onchange_course(self):
        self.batch_id = self.register_id.x_batch
        term_id = False
    ...
    def get_student_vals(self):
        for student in self:
            ...
            details = {
                ...
                'image': student.image,
                'zip': student.zip,
                'x_student_identification_document': student.x_student_identification_document,
                'x_guardian_name': student.x_guardian_name,
                'x_guardian_identification_document': student.x_guardian_identification_document,
                'x_guardian_email': student.x_guardian_email,
                'x_guardian_mobile': student.x_guardian_mobile,
                'x_district': student.x_district
            }
