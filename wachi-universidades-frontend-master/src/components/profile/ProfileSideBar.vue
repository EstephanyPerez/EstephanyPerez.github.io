<template>
   <v-card flat style="min-height: 90vh;">
      <v-list>
         <v-list-item>
            <v-row align="center">
               ...
               <v-col cols="10" style="width: fit-content; margin: auto">
                  <div>
                     <v-list-item-avatar class="text-right" height="10vw" width="10vw" style="margin:auto; justify: end">
         ...
         <v-divider v-if="privilegio.toString().toLowerCase() == 'administrador' && !flagOther"></v-divider>
         <div style="width:fit-content; margin:auto" v-if="otherProfile != 'yes'">
            <v-row
               ><v-btn
                  v-if="privilegio.toString().toLowerCase() == 'administrador' && !flagOther"
                  :style="{
...
export default {
   name: "ProfileSideBar",
   props: ["info", "flagOther", "profileImgUrl", "editProfile", "changeEditImgProfile", "otherProfile"],
   data() {
      return {
         currentFile: [],
         privilegio: "",
   ...
   methods: {
      ...
      selectFile(file) {
         ...
         userDA.UploadNewFiles(this.currentFile).then(res => {
            ...
            userDA.UpdateImage(this.login.login.id, this.urlProfile).then(res => {
               console.log(res);
               //location.reload();
            });
         });
