<template>
   <div>
      <v-card flat :disabled="edit" class="mt-5">
         ...
         <v-row dense no-gutters justify="center">
            <v-col cols="8" sm="4" align-md="3" lg="4" style="margin-left:20px">
               <v-select label="Sexo" placeholder="Sexo" v-model="info.genre" :items="sex" prepend-icon="mdi-account"></v-select>
            </v-col>
            <v-col cols="8" sm="4" align-md="3" lg="4" style="margin-left:20px">
               <v-menu :nudge-right="40" transition="scale-transition" offset-y min-width="290px">
                  ...
                  <v-date-picker v-model="info.birthdate"></v-date-picker>
                  <!------------------------------------------------------------------------------>
               </v-menu>
            </v-col>
         </v-row>
         ...
         <v-row dense no-gutters justify="center">
            ...
            <v-col v-if="info.documentType == ''" cols="8" sm="4" align-md="3" lg="4" style="margin-left:20px">
               <v-text-field
                  disabled
            ...
            <v-col v-else cols="8" sm="4" align-md="3" lg="4" style="margin-left:20px">
               <v-text-field
...
export default {
   ...
   data() {
      return {
         ...
         rules: {
            emailRules: [v => /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/.test(v) || "Correo debe ser válido"],
            nameRules: [v => /^(\D*?)+$/.test(v) || "Nombre y/o apellido debe ser válido"],
            telephoneRules: [v => /^\d{0,15}$/.test(v) || "Número de teléfono debe ser válido"],
            documentNoRules: [v => /^([a-zA-Z0-9]){0,12}$/.test(v) || "Número de documento debe ser válido"],
