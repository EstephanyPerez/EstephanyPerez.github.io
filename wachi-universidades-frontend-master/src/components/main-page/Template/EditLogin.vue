   ...
   <v-main>
      ...
      <v-row no-gutters>
         ...
         <v-col cols="5" style="height: 90vh" class="hidden-md-and-down">
            <v-img lazy-src="https://picsum.photos/id/11/10/6" :src="url" height="100%" alt="Imagen de la tarjeta" :style="{ maxHeight: '90vh' }">
               <input
                  v-if="!read_only"
                  class="file-input"
         ...
         <v-col cols="4" style="height: 90vh;" class="hidden-md-and-down">
            <v-card elevation="0" outlined height="100%" tile :style="{ backgroundColor: secondaryColorDad }">
               <div style="text-align-last:end">
                  <v-btn color="green" fab large dark top right class="mr-5" @click="editComponent()" :style="{ top: '2vh', marginRigth: '3%' }">
                     <v-icon>mdi-pencil</v-icon>
                  </v-btn>
               </div>
               <v-col class="mt-10" style="text-align:center">
                  <google-signin-btn label="Google" customClass="my-button"></google-signin-btn>
               </v-col>
               <div style="padding:6%">
                  ...
                  <v-text-field disabled label="example@email.com" single-line solo></v-text-field>
                  <h4 :style="{ color: generalConfig.colorText }">
                     Contraseña
                  </h4>
                  <v-text-field disabled label="Password" single-line solo append-icon="mdi-eye-off"></v-text-field>
                  <v-btn class="login-button white--text" block elevation="2" :color="mainColorDad">
                     Iniciar Sesión
                  </v-btn>
         ...
         <!-- Tablet/Laptop size -->
         <v-col cols="6" style="height: 90vh" class="hidden-lg-and-up hidden-xs-only">
            <v-img lazy-src="https://picsum.photos/id/11/10/6" :src="url" height="100%" alt="Imagen de la tarjeta" :style="{ maxHeight: '90vh' }">
               <input
                  v-if="!read_only"
                  class="file-input"
         ...
         <v-col cols="6" style="height: 90vh;" class="hidden-lg-and-up hidden-xs-only">
            <v-card elevation="0" outlined height="100%" tile :style="{ backgroundColor: secondaryColorDad }">
               <div style="text-align-last:end">
                  <v-btn color="green" fab large dark top right @click="editComponent()" :style="{ top: '2vh', marginRigth: '3%' }">
                     <v-icon>mdi-pencil</v-icon>
                  </v-btn>
               </div>
               <v-col class="mt-10" style="text-align:center">
         ...
         <v-col cols="12" style="height: 79vh;" class="hidden-sm-and-up">
            <v-card elevation="0" outlined height="100%" tile :style="{ backgroundColor: secondaryColorDad }">
               ...
               />
               <div style="text-align-last:end">
                  <v-btn color="green" fab large dark top right @click="editComponent()" class="mr-5" :style="{ top: '2vh', marginRigth: '3%' }">
                     <v-icon>mdi-pencil</v-icon>
                  </v-btn>
               </div>
               <v-col class="mt-10" style="text-align:center">
      ...
      <v-row justify="center" pa="3" class="mb-5 mt-10">
         <v-dialog v-model="dialog_login" persistent max-width="490">
            ...
            <v-card>
               ...
               <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="red darken-1" text @click="dialog_login = false">
                     Cancelar
                  </v-btn>
                  <v-btn color="green darken-1" text @click="[(dialog_login = false), updateMB()]">
                     Aceptar
                  </v-btn>
               </v-card-actions>
            </v-card>
...
export default {
   ...
   methods: {
      ...
      onSelectFile() {
         ...
         if (filesize < 2) {
            ...
            Swal.fire({
               icon: "error",
               title: '<p style="font-family:Roboto;">Error</p>',
               html: '<p style="font-family:Roboto;">Tamaño de archivo muy grande! Max. 2MB</p>'
            });
            return;
         }
      },
      ...
      createBlueprintJson() {
         ...
         if (this.imgLogin != null && this.imgLogin != "" && this.imgLogin != "imagen" && this.imgLogin.size > 0) {
               ...
               if (res.status === 200) {
                  blueprintDA.sendImagesBlueprints(formData, data.communityToken).then(res => {
                     if (res.status == 200) {
                        ...
                        Swal.fire({
                           icon: "success",
                           title: '<p style="font-family:Roboto;">Login actualizado</p>',
                           html: '<p style="font-family:Roboto;">Se actualizo el login</p>'
                        });
                     }
                  });
               }
