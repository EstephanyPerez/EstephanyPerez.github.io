<template>
   <v-card v-if="this.c1_content.editComponent1">
      ...
      <v-card-text>
         ...
         <v-textarea solo outlined name="input-7-4" v-model="landingConfig.landingText.text" auto-grow counter="50" :maxlength="50"></v-textarea>

         <p>Texto del botón</p>
         <v-text-field dense outlined v-model="landingConfig.landingText.buttonText" counter="20" :maxlength="20"></v-text-field>
      </v-card-text>
   </v-card>
</template>
...
export default {
   ...
   computed: {
      ...mapState(["landingConfig", "generalConfig", "c_editComponent1", "landingImage"])
   },
   methods: {
      ...mapActions(["updateComponent1Content", "editComponent1", "updateLandingImage"]),
      updateContent() {
