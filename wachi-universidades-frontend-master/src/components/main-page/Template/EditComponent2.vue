   ...
   <v-card v-if="this.c_editComponent2" :style="{ height: '100%' }">
      ...
      <v-card-text>
         ...
         <div>
            ...
            <v-expansion-panels>
               <v-expansion-panel v-for="(item, i) in multiCardConfig.cards" :key="i">
                  <v-expansion-panel-header> Tarjeta {{ i + 1 }} </v-expansion-panel-header>
                  <v-expansion-panel-content>
                     <div>
                        <div>
                           <h4>Titulo de la tarjeta</h4>
                           <div :id="'title-card-edit' + i" :style="{ display: flagParts[i].title.display }">
                              <v-textarea
                                 class="edit-input"
                                 solo
                                 name="input-7-4"
                                 ...
                                 @blur="leaveSelectedItem('title-card-text' + i)"
                              ></v-textarea>
                           </div>
                        </div>
                        ...
                        <div>
                           ...
                           <div :id="'text-card-edit' + i" :style="{ display: flagParts[i].text.display }">
                              <v-textarea
                                 class="edit-input"
                                 solo
                                 name="input-7-4"
                        ...
                        <div>
                           ...
                           ></v-checkbox>
                           <div :id="'image-card-edit' + i" :style="{ display: flagParts[i].image.display }"></div>
                        </div>
                     </div>
                  </v-expansion-panel-content>
               </v-expansion-panel>
...
export default {
   ...
   computed: {
      ...mapState(["multiCardConfig", "c_editComponent1", "c_editComponent2", "c_editComponent3", "multicard0", "multicard1", "multicard2"])
   },
   updated() {
   ...
   beforeMount() {
      this.multiCardConfig.cards.map((item, index) => {
         ...
         this.flagParts.push({
            title: typeof item.title == "undefined" ? { display: "none", flag: false } : { display: "block", flag: true },
            text: typeof item.text == "undefined" ? { display: "none", flag: false } : { display: "block", flag: true },
            image: typeof item.image == "undefined" ? { display: "none", flag: false } : { display: "block", flag: true }
         });
   ...
   methods: {
      ...mapActions(["editComponent1", "updateMulticardImg0", "updateMulticardImg1", "updateMulticardImg2"]),
      previewImage(pos) {
         if (pos == 0) {
            this.updateMulticardImg0(this.imgBackground[0]);
            //console.log(this.multicard0);
            this.image.multiCard1 = URL.createObjectURL(this.imgBackground[pos]);
         } else if (pos == 1) {
      ...
      handleSelect(element, pos) {
         ...
         if (element.localeCompare("title") == 0) {
            if (!this.flagParts[pos].title.flag) {
               document.getElementById("title-card-text" + pos).style.display = "none";
               ...
               document.getElementById("title-card-text" + pos).style.display = "block";
            }
         } else if (element.localeCompare("text") == 0) {
            console.log(this.flagParts[pos].text);
            if (!this.flagParts[pos].text.flag) {
               document.getElementById("body-card-text" + pos).style.display = "none";
               ...
               document.getElementById("body-card-text" + pos).style.display = "block";
            }
         } else if (element.localeCompare("image") == 0) {
            console.log(this.flagParts[pos].image);
            if (!this.flagParts[pos].image.flag) {
               document.getElementById("image-card-text" + pos).style.display = "none";
               ...
               document.getElementById("image-card-text" + pos).style.display = "block";
            }
         }
      }
   }
