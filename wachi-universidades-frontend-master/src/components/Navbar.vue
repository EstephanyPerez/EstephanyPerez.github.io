<template>
   ...
   <v-main>
      ...
      <v-navigation-drawer v-model="drawer" class="d-sm-none" app fixed clipped-left absolute floating temporary offset-y>
         <v-list nav dense>
            <!-- CELLPHONE  ===============================-->

            <v-list-item>
               <v-row align="center" style="padding-bottom: 0px" v-if="showEditButton">
                  <v-col cols="2">
                     <v-file-input
                  ...
                  <v-col cols="10" style="width: fit-content; margin: auto" v-if="otherProfile != 'yes' && showEditButton">
                  ...
                  <v-col cols="12" style="width: fit-content; margin: auto" v-else>
                     <div style="width: fit-content; margin: auto">
                        <v-list-item-avatar
                           class="text-right"
                           height="40vw"
                           ...
                           style="margin: auto; justify: end"
                        >
                           <v-img v-if="showEditButton" :src="urlProfile"> </v-img>
                           <v-img v-else src="https://assets-2flock.s3.amazonaws.com/ec84b565-011c-4ad8-a1c1-403c1b6eac5c/1606695642121.png"> </v-img>
                        </v-list-item-avatar>
                     </div>
                  </v-col>
               </v-row>
            ...
            <v-list-item v-if="!getUser().session.authenticated" text :key="navbarConfig.login.title" :to="navbarConfig.login.link">
               <v-list-item-icon>
                  <v-icon>{{ navbarConfig.login.icon }}</v-icon>
               </v-list-item-icon>
               <v-list-item-title>{{ navbarConfig.login.title }}</v-list-item-title>
            </v-list-item>
            <v-list-item v-if="getUser().session.authenticated" class="px-2">
               <v-list-item-avatar>
                  <v-img :src="navbarConfig.profilePhoto" :color="generalConfig.mainColor" alt="Perfil"></v-img>
               </v-list-item-avatar>
            </v-list-item>
            <v-divider></v-divider>
            <v-list-item-group>
               <v-list-item v-for="item in navbarConfig.menuItems" text :key="item.title" :to="item.link" @click="navbarClickActions(item.action)">
                  <v-list-item-icon>
               ...
               >
                  ...
                  <v-list-item-title>{{ navbarConfig.shoppingCart.title }}</v-list-item-title>
               </v-list-item>
            </v-list-item-group>
            <v-divider></v-divider>
            <v-list-item-group>
               <v-list-item v-for="item in navbarConfig.dropDown" text :key="item.title" :to="item.link" @click="navbarClickActions(item.action)">
                  <v-list-item-icon v-if="item.title != 'Mis compras' || item.title != 'Mis ventas'">
                     <v-icon>{{ item.icon }}</v-icon>
                  </v-list-item-icon>
                  <v-list-item-title v-if="item.title != 'Mis compras' || item.title != 'Mis ventas'">{{ item.title }}</v-list-item-title>
                  <v-list-item-group v-if="item.title == 'Mis compras' || item.title == 'Mis ventas'">
      ...
      <v-app-bar class="d-none d-sm-block" flat elevate-on-scroll>
         ...
         <v-toolbar-items>
            <v-btn v-for="item in navbarConfig.menuItems" text :key="item.title" :to="item.link" @click="navbarClickActions(item.action)">
               <span>{{ item.title }}</span>
            </v-btn>
            <v-btn v-if="communityType_local == 'marketplace'" icon color="black">
               <v-icon>{{ navbarConfig.shoppingCart.icon }}</v-icon>
            </v-btn>
            <v-btn
            ...
            <v-menu v-else offset-y>
               ...
               <v-list>
                  <v-list-item v-for="(item, i) in navbarConfig.dropDown" :key="i" :to="item.link" @click="navbarClickActions(item.action)">
                     <v-list-item-title>{{ item.title }}</v-list-item-title>
                  </v-list-item>
               </v-list>
            </v-menu>
         ...
         <v-menu class="hidden-md-and-up">
            <v-list>
               <v-list-tile v-for="item in navbarConfig.menuItems" :key="item.title" :to="item.link">
                  <v-list-tile-content>
...
export default {
   name: "Navbar",
   props: ["profileBool", "otherProfile", "flag", "flagOther", "profileImgUrl", "editProfile", "changeEditImgProfile"],
   data() {
      return {
         currentFile: [],
         drawer: false,
         communityType_local: "university", // Update Navbar components by Community
         navbarConfig: {
            menuItems: [],
            login: {
   ...
   computed: {
      ...mapState(["generalConfig", "urlLogo", "communityType", "login"])
   },
   created() {
      this.urlProfile = localStorage.getItem("avatarURL");
      this.userName = localStorage.getItem("userName");
      this.userLastName = localStorage.getItem("userLastName");
      this.privilegio = this.login.login.privilege;
   ...
   methods: {
      ...
      selectFile(file) {
         ...
         userDA.UploadNewFiles(this.currentFile).then(res => {
            console.log(res);
            this.urlProfile = res.data.payload.urls[0];
            localStorage.setItem("avatarURL", this.urlProfile);

            userDA.UpdateImage(user.data.id, this.urlProfile).then(res => {
               console.log(res);
               location.reload();
            });
         });
      ...
      fillNavbarOptions(commId) {
         ...
         3 : Marketplace
         */
         if (["entrepreneurship", "university", "marketplace"].includes(commId)) {
            if (this.getUser().session.authenticated) {
                  ...
                  if (!this.flag) {
                     ...
                     this.navbarConfig.dropDown.push(
                        ...
                        },
                        {
                           icon: "mdi-file-document-edit-outline",
                           title: "Mis ventas",
                           items: [{ title: "Publicaciones" }, { title: "Pedidos" }, { title: "Preguntas" }, { title: "Reportes" }]
                        }
                     );
                  }
               }
               ...
               if (this.privilegio.toString().toLowerCase() == "administrador" && !this.flagOther) {
                  if (!this.flag) {
                     this.navbarConfig.dropDown.push({
                        icon: "mdi-pencil",
      ...
      navbarClickActions(action) {
         ...
         switch (action) {
            ...
            case "edit-password":
               ...
               break;
            case "myProfile":
               if (this.profileBool != undefined && this.profileBool) location.reload();
               break;
         }
      },
      onFileChange() {
