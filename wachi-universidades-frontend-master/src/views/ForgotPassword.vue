            ...
            <v-row align="center" justify="center">
               ...
               >
                  <div class="pa-md-10 mx-lg-auto" width="5px">
                     <label class="font-weight-bold">Cambio de contraseña</label>
                     <v-card
                        flat
                        tile
                        :style="{
                     ...
                     <v-dialog v-model="sendingCode" persistent max-width="600" height="200px">
                        <v-card flat tile>
                           <v-card-title>
                              <label class="font-weight-bold">Enviar código de validación </label>
                           </v-card-title>
                           <v-card-text style="padding-bottom: 0px">
                              <v-container>
                                 <v-text-field label="Correo" placeholder="ejemplo@wachi.com" v-model="data.email"></v-text-field>
                              </v-container>
                           </v-card-text>
                     ...
                     <v-row class="pa-1">
                        <v-col>
                           <v-text-field label="Código de validación" placeholder="Código de validación" v-model="data.code"></v-text-field>
                        </v-col>
                     </v-row>
...
export default {
   ...
   methods: {
      changePassword() {
         if (this.data.email == null || this.data.email.trim() == "") {
         ...
         } else if (this.data.email.match(/^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/) == null) {
            Swal.fire({
               title: '<p style="font-family:Roboto;">Error</p>',
               icon: "error",
               html: '<p style="font-family:Roboto;">Ingrese un correo válido</p>' //CHECK THE STANDARIZATION OF THINGSS
            });
         } else if (this.data.newPassword == null || this.data.newPassword == "") {
            Swal.fire({
               title: '<p style="font-family:Roboto;">Error</p>',
               icon: "error",
            ...
            Swal.fire({
               title: '<p style="font-family:Roboto;">Error</p>',
               icon: "error",
               html: '<p style="font-family:Roboto;">No coinciden las contraseñas ingresadas</p>' //CHECK THE STANDARIZATION OF THINGSS
            });
         } else if (this.data.code == null || this.data.code == "") {
            Swal.fire({
               title: '<p style="font-family:Roboto;">Error</p>',
               icon: "error",
               html: '<p style="font-family:Roboto;">Ingrese un código de validación</p>' //CHECK THE STANDARIZATION OF THINGSS
            });
         } else {
            userDA
               ...
               .catch(function(error) {
                  ...
                  Swal.fire({
                     title: '<p style="font-family:Roboto;">Error</p>',
                     icon: "error",
                     html: '<p style="font-family:Roboto;">No se pudo cambiar la contraseña</p>' //CHECK THE STANDARIZATION OF THINGSS
                  });
               });
         }
      },
