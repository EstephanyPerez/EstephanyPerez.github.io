   ...
   <div>
      ...
      <v-row>
         ...
         <v-col cols="8">
            <v-row>
               <v-col cols="12">
                  <h1>GESTIÓN DE COORDINADORES DE SEDE</h1>
               </v-col>
               <v-col cols="8"></v-col>
               <v-col cols="4"
                              ...
                              <v-dialog v-model="dialogForm" max-width="700px">
                                 <template v-slot:activator="{ on, attrs }">
                                    <div v-bind="attrs" v-on="on">
                                       ...
                                       <span>Nuevo Coordinador de Sede</span>
                                    </div>
                                 </template>
                                 <v-card>
                                    <v-card-title class="justify-center">
                                       <h4>NUEVO COORDINADOR DE SEDE</h4>
                                    </v-card-title>
                                    <v-divider></v-divider>
                                    <v-card-text>
                                       <v-form>
                        ...
                        <v-menu offset-y>
                           ...
                           <v-list>
                              <v-dialog v-model="deleteCoordinator" width="800px">
                                 <template v-slot:activator="{ on, attrs }">
                                    <v-list-item v-bind="attrs" v-on="on">
                                       <v-list-item-title>Eliminar Coordinador</v-list-item-title>
                                    </v-list-item>
                                 </template>
                                 <v-card>
                                    <v-card-title>
                                       <h2>Eliminar Coordinador de sede</h2>
                                    </v-card-title>
                                    <v-spacer></v-spacer>
                                    <v-card-text>
                                       <v-form>
                                          <v-row align="center" justify="center">
                                             <v-col cols="12">
                                                <div class="text-center">
                                                   <h2>
                                                      ¿Está seguro que desea eliminar el Coordinador de sede
                              ...
                              <v-dialog v-model="modifyCoordinator" width="800px">
                                 <template v-slot:activator="{ on, attrs }">
                                    <v-list-item v-bind="attrs" v-on="on">
                                       <v-list-item-title>Editar Coordinador de Sede</v-list-item-title>
                                    </v-list-item>
                                 </template>
                                 <v-card>
                                    <v-card-title>
                                       <h2>Editar Coordinador de Sede</h2>
                                    </v-card-title>
                                    <v-spacer></v-spacer>
                                    <v-card-text>
                                       <v-form>
...
export default {
   ...
   computed: {
      ...
      newNameErrors() {
         ...
         !this.$v.newCoordinator.name.required && errors.push("Ingresar el nombre de la fase");
         return errors;
      },
      newEmailErrors() {
         const errors = [];
         ...
         !this.$v.newCoordinator.email.required && errors.push("Ingresar la descripción de la fase");
         return errors;
      },
      newLastNameErrors() {
         const errors = [];
         if (!this.$v.newCoordinator.lastName.$dirty) return errors;
         !this.$v.newCoordinator.lastName.required && errors.push("Ingresar el orden de la fase");
         return errors;
      },
   ...
   methods: {
      deleteSeletectedCoordinator(item) {
         ...
         RolesDA.deleteAdmin(item.idUserDomain)
            .then((resp) => {
               console.log(resp);
               this.$swal.fire(
                  "El coordinador de sede se ha eliminado del evento",
                  "Se ha eliminado el coordinador de sede " + item.name + " " + item.lastName + " en el evento con éxito",
                  "success"
               );
               let index;
            ...
            .catch((err) => {
               this.$swal.fire(
                  "Error al eliminar el coordinador de sede",
                  "No se ha podido eliminar el coordinador de sede " + item.name + " " + item.lastName,
                  "error"
               );
      ...
      registerCoordinator() {
         ...
         if (!this.$v.newCoordinator.$invalid) {
            ...
            console.log("Coordinador: ", this.newCoordinator);
            let body = {
            ...
            RolesDA.registerNewRole(this.currentEvent.idUEvent, body)
               .then((resp) => {
                  ...
                  if (resp.data.payload.password) {
                     this.$swal.fire(
                        "Coordinador de Sede creado en el evento",
                        "Se ha creado el coordinador de sede " + this.newCoordinator.name + " en el evento con éxito " + resp.data.payload.password,
                        "success"
                     );
                  } else {
                     this.$swal.fire(
                        "Coordinador de Sede creado en el evento",
                        "Se ha creado el coordinador de sede " + this.newCoordinator.name + " en el evento con éxito ",
                        "success"
                     );
                  }
               ...
               .catch((err) => {
                  this.$swal.fire("Ocurrió un problema", "No se pudo registrar el coordinador de sede en el evento", "error");
                  console.log(err.response.data);
               });
         }
      },
      changeCoordinator(item) {
         console.log("Modificando el coordinador con id: ", item.idUserDomain);
         let body = {
         ...
         RolesDA.editAdminRole(body)
            .then((resp) => {
               console.log(resp);
               this.$swal.fire(
                  "Coordinador de Sede Modificado",
                  "Se ha modificado el coordinador de sede " + item.name + " " + item.lastName + "en el evento con éxito",
                  "success"
               );
            })
            .catch((err) => {
               this.$swal.fire("Ocurrió un problema", "No se pudo modificar el coordinador de sede en el evento", "error");
               console.log(err.response.data);
            });
      },
      namesFilter(value) {
         ...
         RolesDA.getAdminRoles(this.currentEvent.idUEvent)
            .then((resp) => {
               ...
               for (let i = 0; i < resp.data.payload.length; i++) {
                  if (resp.data.payload[i].privilege == "coordinador de sede") {
                     let auxCoordinator = {
                  ...
                  //console.log("coordinadores: ", this.coordinatorFilter);
                  //console.log("coordinadores: ", this.coordinators);
