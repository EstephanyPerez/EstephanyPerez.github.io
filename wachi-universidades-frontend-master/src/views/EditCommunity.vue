         ...
         <v-card class="mx-auto ma-5" max-width="100%" elevation="0">
            <div v-if="!this.loading">
               <div v-if="itemNavBar.localeCompare('editMaingPage') === 0">
                  <Template :flagToUpdate="flagToUpdate" v-if="itemNavBar.localeCompare('editMaingPage') === 0" />
               </div>
               <div v-if="itemNavBar.localeCompare('editLogin') === 0">
                  <EditLogin :datos="edit" :flagToUpdate="flagToUpdate" v-if="itemNavBar.localeCompare('editLogin') === 0" />
               </div>
               <div v-if="itemNavBar.localeCompare('editDatosComunidad') === 0">
                  <DataView :datos="edit" :flagToUpdate="flagToUpdate" v-if="itemNavBar.localeCompare('editDatosComunidad') === 0" />
               </div>
            </div>
            <div v-if="this.loading">
               {{ textLoading }}
...
import ToolbarEdit from "@/components/main-page/Template/ToolbarEdit";
import { mapActions } from "vuex";
import Template from "@/components/main-page/Template/Template";
import EditLogin from "@/components/main-page/Template/EditLogin";
import DataView from "@/components/main-page/Template/DataView";
